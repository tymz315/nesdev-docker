...
putbyte(int offset, int data)
{
	int b=0;
	if (section == S_EMU) {
		emulator_memory[offset&0xFFFF] = (data) & 0xFF;
		return;
	}
	if (bank >= RESERVED_BANK)
		return;
	while(offset>=0x2000 && bank+b<127 && bank_name[bank+b][0] && !strcmp(bank_name[bank+b],bank_name[bank+b+1])) b++,offset-=0x2000;
	if (offset < 0x2000) {
		rom[bank+b][offset] = (data) & 0xFF;
		map[bank+b][offset] = section + ((page+b) << 5) + pseudopage;
...
putword(int offset, int data)
{
	...
	if (section == S_EMU) {
		emulator_memory[offset&0xFFFF] = (data) & 0xFF;
		emulator_memory[(offset+1)&0xFFFF] = (data >> 8) & 0xFF;
		return;
	}
	if (bank >= RESERVED_BANK)
		return;
	...
	if (offset < 0x1FFF) {
		...
		/* high byte */
		rom[bank+b][offset+1] = (data >> 8) & 0xFF;
		map[bank+b][offset+1] = section + ((page+b) << 5) + pseudopage;

		/* update rom size */
		if (bank > max_bank)
			max_bank = bank;
	}
	if (offset == 0x1FFF && bank<127 && bank_name[bank][0] && !strcmp(bank_name[bank],bank_name[bank+1])) {
		rom[bank+b][offset] = (data) & 0xFF;
		map[bank+b][offset] = section + ((page+b) << 5) + pseudopage;
		rom[bank+b+1][0] = (data >> 8) & 0xFF;
		map[bank+b+1][0] = section + ((page+b+1) << 5) + pseudopage;
		if (bank+b+1 > max_bank)
			max_bank = bank+b+1;
	}
}
...
putbuffer(void *data, int size)
{
	...
	if (section == S_EMU) {
		if (loccnt + size > 0xFFFF) {
			fatal_error("Emulator memory overflow!");
			return;
		}
		/* copy the buffer */
		if (pass == LAST_PASS) {
			if (data) {
				memcpy(emulator_memory+loccnt, data, size);
			}
			else {
				memset(emulator_memory+loccnt, 0, size);
			}
		}
		loccnt += size;
		return;
