...
extern int pseudopage;	/* pseudopage */
extern int rsbase;	/* .rs counter */
extern int section;	/* current section: S_ZP, S_BSS, S_CODE, S_DATA, or S_EMU */
...
extern struct t_symbol *bank_glabl[5][256];	/* latest global label in each bank */
...
extern int  opvaltab[7][16];

extern unsigned char emulator_memory[0x10000];
extern FILE *emu_fp;
extern int use_custom_output;	/* emulator is used to make output file */
extern FILE*emu_datafiles[32];
extern int emu_datafiles_count;

#define CHAIN_BANKS while(loccnt>=0x2000 && bank<127 && bank_name[bank][0] && !strcmp(bank_name[bank],bank_name[bank+1])) bank++,page++,loccnt-=0x2000
