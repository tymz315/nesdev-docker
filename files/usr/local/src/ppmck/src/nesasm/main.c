...
static void write_map_output(void);
...
main(int argc, char **argv)
{
	...
	for (pass = FIRST_PASS; pass <= LAST_PASS; pass++) {
		...
		/* .emu */
		section_bank[S_EMU]          = 0x00;
		bank_page[S_EMU][0x00]       = 0x00;
		bank_loccnt[S_EMU][0x00]     = 0x8000;
		...
		while (readline() != -1) {
			assemble();
			CHAIN_BANKS;
			if (loccnt > 0x2000 && section != S_EMU) {
				if (proc_ptr == NULL)
	...
	if(use_custom_output) {
		if ((emu_fp = fopen(bin_fname, "wb")) == NULL) {
	...
	if(emu_fp) fclose(emu_fp);

	/* write map output */
	if(writemap_opt) write_map_output();
...
help(void)
{
	...
	printf("-o #   : override output filename\n");
	...
	printf("-.nes  : select NES/Famicom machine type\n");
	printf("-.pce  : select PC-Engine machine type\n");
...
show_seg_usage(void)
{
	...
	for (i = 0; i <= max_bank; i++) {
		...
		for (;;) {
			...
			/* get section type */
			section = map[i][addr] & 0x03;
			page = (map[i][addr] & 0xE0) << 8;
			start = addr;

			/* search section end */
			for (; addr < 8192; addr++)
				if ((map[i][addr] & 0x03) != section)
					break;
 ...
 */
static void write_map_output(void) {
  FILE*fp=fopen(map_fname,"wb");
  int i;
  struct t_symbol*sym;
  fputc(max_zp&255,fp); fputc(max_zp>>8,fp);
  fputc(max_bss&255,fp); fputc(max_bss>>8,fp);
  fputc(machine->type,fp);
  fputc(max_bank,fp);
  ...
  fputc(0,fp); fputc(0,fp);
  fputc(0,fp); fputc(0,fp);
  fwrite(emulator_memory,1,0x2000,fp);
  fwrite(map,8192,max_bank+1,fp);
  for(i=0;i<=max_bank;i++) fwrite(bank_name[i],1,16,fp);
  for(i=0;i<256;i++) {
    sym=hash_tbl[i];
    while(sym) {
      fputc(sym->reserved,fp);
      fputc(sym->type,fp);
      fputc(sym->value&255,fp); fputc(sym->value>>8,fp);
      fputc(sym->bank,fp);
      fputc(sym->page,fp);
      fputc(sym->size&255,fp); fputc(sym->size>>8,fp);
      fputc(sym->data_type,fp);
      fputc(sym->data_size&255,fp); fputc(sym->data_size>>8,fp);
      fwrite(sym->name,1,sym->name[0]+1,fp);
      //if(sym->values) fwrite(sym->values,sizeof(int),sym->size,fp);
      sym=sym->next;
    }
  }
  fclose(fp);
