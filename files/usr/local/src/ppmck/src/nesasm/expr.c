...
static int push_val_direct(unsigned int val)
{
...
cont:
	...
	while (!end) {
		...
		/* operators */
			...
			/* divide, counted label */
			case '/':
				if (!need_operator) {
					if(!getsym()) return 0;
					expr_lablcnt++;
					if(!( expr_lablptr = stlook(1) )) return 0;
					if(expr_lablptr->type != COUNTED) return 0;
					arg = 0;
					for(;;) {
						if(*expr=='+') arg++;
						else if(*expr=='-') arg--;
					...
					if(arg<0) arg++;
					arg += expr_lablptr->value - 1;
					if(*expr=='?') {
						push_val_direct(arg);
						expr++;
					} else if(*expr=='#') {
						push_val_direct(expr_lablptr->size);
						expr++;
					} else {
						if(arg<0 || arg>=2048 || (pass==LAST_PASS && arg>=expr_lablptr->size)) {
							error("Counted label too far!");
							return 0;
						}
						push_val_direct(expr_lablptr->values[arg]);
					}
					break;
				}
				/* fall through */
			...
			case '?':
				...
				expr++;
				c = *expr++;
				if(c=='A') push_val_direct(bank_base);
				if(c=='B') push_val_direct(bank);
				if(c=='C') push_val_direct(section);
				if(c=='D') push_val_direct(data_loccnt);
				if(c=='E') push_val_direct(errcnt);
				if(c=='L') push_val_direct(loccnt);
				if(c=='M') push_val_direct(bank<128 && section!=S_EMU?map[bank][loccnt&0x1FFF]:0);
				if(c=='P') push_val_direct(page);
				if(c=='Q') push_val_direct(pseudopage);
				if(c=='R') push_val_direct(rsbase);
				if(c=='S') push_val_direct(slnum);
				if(c=='X') push_val_direct(pass);
				if(c=='Z') push_val_direct(max_bank);
				break;
...
push_val(int type)
{
	...
	/* decimal number 48 (or hexa 0x5F) (or octal 0o177) */
	case T_DECIMAL:
		if((c == '0') && (expr[1] == 'o')) {
			mul = 8;
			expr++;
		}
		else if((c == '0') && (toupper(expr[1]) == 'X')) {
			mul = 16;
			expr++;
		}
		else {
...
getsym(void)
{
	...
	while (valid) {
		c = *expr;
		if (isalpha(c) || c == '_' || c == '.' || (isdigit(c) && i >= 1) || ( (local_check=='.') && ((c=='-') || (c=='+')) ) || ( c=='/' && !i )) {
			if (i < SBOLSZ - 1)
				symbol[++i] = c;
			expr++;
		}
