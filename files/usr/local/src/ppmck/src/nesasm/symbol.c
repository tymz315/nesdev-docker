...
colsym(int *ip)
{
	...
	for (;;) {
		...
		if (isdigit(c) && (i == 0))
			break;
		if ((!isalnum(c)) && (c != '_') && (c != '.') && !( (local_check=='.') && ((c=='-') || (c=='+')) ) && !( i==0 && c=='/' ))
			break;
		if (i < (SBOLSZ - 1))
			symbol[++i] = c;
		(*ip)++;
...
struct t_symbol *stinstall(int hash, int type)
{
	...
	sym->type  = (symbol[1] == '/') ? COUNTED : (if_expr ? IFUNDEF : UNDEF);
	sym->value = 0;
	sym->local = NULL;
	sym->proc  = NULL;
	...
	sym->pal   = -1;
	sym->refcnt = 0;
	sym->reserved = (symbol[1] == '/');
	sym->data_type = -1;
	sym->data_size = 0;
	strcpy(sym->name, symbol);
	sym->values = NULL;
	if(symbol[1] == '/') {
		if(!( sym->values = malloc(2048*sizeof(int)) )) fatal_error("Out of memory!");;
	}
...
labldef(int lval, int flag)
{
	...
	if (flag && section != S_EMU)
		lval = (lval & 0x1FFF) | (page << 13);

	/* counted labels */
	if(lablptr->type == COUNTED) {
		if(lablptr->reserved != pass) {
			lablptr->size = lablptr->value; /* reuse "size" flag */
			lablptr->value = 0;
			lablptr->reserved = pass; /* reuse "reserved" flag for other purpose */
		}
		if(lablptr->value >= 2048) {
			fatal_error("Too many uses of a counted label!");
			return (-1);
		} else {
			lablptr->values[lablptr->value++] = lval;
		}
	}
	...
	/* second pass */
		...
		{
			fatal_error("Symbol value changed between passes!");
			return (-1);
		}
	}
	...
	if (flag) {
		...
		if(section == S_BSS || section == S_ZP) lablptr->bank = bank;
		else lablptr->bank = bank_base + bank;
		lablptr->page = page;
