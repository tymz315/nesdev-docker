...
char pseudo_flag[] = {
	0x1C, 0x1C, 0x1F, 0x1F, 0x1F, 0x1C, 0x1C, 0x1C, 0x0F, 0x0C,
	0x0C, 0x0C, 0x0C, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F,
	0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x0C,
	0x1F, 0x1F, 0x1F, 0x0C, 0x0C, 0x0C, 0x0C, 0x1F, 0x1F, 0x1F,
	0x1F, 0x1F, 0x0C, 0x0C, 0x0C, 0x04, 0x04, 0x04, 0x04, 0x04,
...
do_db(int *ip)
{
	...
	for (;;) {
		/* ASCII string */
		if (prlnbuf[*ip] == '\"' || prlnbuf[*ip] == '|') {
			t = (prlnbuf[*ip] == '|');
			if(t) {
				c = prlnbuf[++(*ip)];
				if(c != '"') {
					error("Uninitiated ASCII string!");
					return;
				}
			}
			for (;;) {
				...
				if(t) c = emulator_memory[0x2000|(127&++(*ip))];
				/* store char on last pass */
				if (pass == LAST_PASS || section == S_EMU)
					putbyte(loccnt, c);
...
do_org(int *ip)
{
	...
	if(section == S_EMU) loccnt = value & 0xFFFF;
	else loccnt = (value & 0x1FFF);
...
do_incbin(int *ip)
{
	...
	if(section == S_EMU) {
		if(loccnt + size > 0x10000) {
			fclose(fp);
			error("Emulation memory overflow!");
			return;
		}
		fread(emulator_memory+loccnt, 1, size, fp);
		...
		if (pass == LAST_PASS) {
			...
			memset(&map[bank][loccnt], section + (page << 5) + pseudopage, size);
...
do_ds(int *ip)
{
	...
	case S_DATA:
		...
		limit = 0x2000;
		if(bank<127 && bank_name[bank][0] && !strcmp(bank_name[bank],bank_name[bank+1])) limit = 0x4000;
		break;
	...
	if (pass == LAST_PASS) {
		...
		case S_DATA:
			...
			memset(&map[bank][loccnt], section + (page << 5) + pseudopage, value);
			if (bank > max_bank)
				max_bank = bank;
			break;
		}
	...
	/* update location counter */
	loccnt += value;
	if(section == S_EMU) loccnt &= 0xFFFF;
}
...
do_section(int *ip)
{
	...
	/* check if custom output enable for .EMU section */
	if (section == S_EMU) {
		...
		if(prlnbuf[*ip]=='X' || prlnbuf[*ip]=='x') use_custom_output=1;
	} else {
	/* check for pseudopage numbers */
		/* skip spaces */
		while (isspace(prlnbuf[*ip]))
			(*ip)++;
		if (prlnbuf[*ip]<=32 || !evaluate(ip, ';')) pseudopage=0;
		else pseudopage=value;
	}
 ...
 * do_assign()
 * ----
 * .assign pseudo - reassign values to reserved symbols
 */
 ...
 * do_macgoto()
 * ----
 * .macgoto pseudo - tail call to another macro using same parameters
 */
...
{
	if(!expand_macro) {
		error("Don't MACGOTO outside of macros!");
		return;
	}
	while(prlnbuf[*ip] == ' ') (*ip)++;
	mptr = macro_look(ip);
 ...
 * do_macset()
 * ----
 * .macset pseudo - adjust macro arguments
 */
...
do_macset(int *ip)
{
	...
	if(!expand_macro) {
		error("Don't MACSET outside of macros!");
		return;
	}
	...
	switch(y) {
		case 1: // Copy argument of inside called macro
			if(midx<8 && z>0 && z<=9) memcpy(marg[midx][x],marg[midx+1][z-1],80);
			break;
		case 2: // Copy argument of current macro
			if(z>0 && z<=9 && x!=z-1) memcpy(marg[midx][x],marg[midx][z-1],80);
			break;
		case 3: // Single character
			...
			break;
		case 4: // Numeric
			sprintf(marg[midx][x],"%i",z);
			break;
		case 5: // Cut off by string length
			if(z>=0 && z<80) marg[midx][x][z]=0;
			break;
		case 6: // ASCII code of character at position
			if(z>=0 && z<80) sprintf(marg[midx][x],"%i",marg[midx][x][z]*(strlen(marg[midx][x])>z));
			break;
		case 7: // Set and increment mcounter
			sprintf(marg[midx][x],"%i",mcounter+=z);
			break;
		case 8: // Read data in ROM bank
			if(bank<128) sprintf(marg[midx][x],"%i",rom[bank][z&0x1FFF]);
			break;
		case 9: // Bank name
			memcpy(marg[midx][x],bank_name[z&127],63);
			marg[midx][x][64]=0;
			break;
		case 10: // Read data in map of ROM bank
			if(bank<128) sprintf(marg[midx][x],"%i",map[bank][z&0x1FFF]);
			break;
		case 11: // Read string from stdin
            fgets(marg[midx][x],80,stdin);
            if(marg[midx][x][0] && marg[midx][x][strlen(marg[midx][x]+1)]=='\n') marg[midx][x][strlen(marg[midx][x]+1)]=0;
            if(marg[midx][x][0] && marg[midx][x][strlen(marg[midx][x]+1)]=='\r') marg[midx][x][strlen(marg[midx][x]+1)]=0;
			break;
		case 12: // Affect IF nesting levels
            sprintf(marg[midx][x],"%i",if_level);
            if_level+=y;
            skip_lines=z;
            in_if=!!if_level;
            if(if_level<0 || if_level>255) {
 ...
 * do_datafile()
 * ----
 * .datafile pseudo - load extra data file
 */
...
do_datafile(int *ip)
{
	...
	/* define label */
	labldef(emu_datafiles_count, 0);

	if(emu_datafiles_count>=32) {
		fatal_error("Too many data files!");
		return;
	}
	...
	/* open file */
	if (!(emu_datafiles[emu_datafiles_count++]=fopen(fname,"rb"))) {
		fatal_error("Can not open file!");
		return;
	}
}
