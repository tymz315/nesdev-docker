...
sound_rts_low		.ds	1		;Improved PPMCK:
sound_rts_high		.ds	1		;return from subroutine
			.ds	CH_COUNT*2 - 2
sound_rts_bank		.ds	1		;
			.ds	CH_COUNT*2 - 1
...
bank_address_change:
	...
	if (ALLOW_BANK_SWITCH)
	lda	sound_bank,x		; support return from subroutine
	sta	sound_rts_bank,x	; support return from subroutine
	endif
	lda	<sound_add_high,x	; support return from subroutine
	sta	sound_rts_high,x	; support return from subroutine
	lda	<sound_add_low,x	; support return from subroutine
	sta	sound_rts_low,x		; support return from subroutine
	...
	rts
;-------------------------------------------------------------------------------
;Improved PPMCK: Go to return address
data_return:
	if (ALLOW_BANK_SWITCH)
	lda	sound_rts_bank,x
	sta	sound_bank,x
	endif

	lda	sound_rts_high,x
	sta	<sound_add_high,x
	lda	sound_rts_low,x
	sta	<sound_add_low,x

	jmp	sound_data_address ;rts
;-------------------------------------------------------------------------------
;data_end_sub:
;	ldy	<channel_sel
;	
